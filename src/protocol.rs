...
fn dispatch_authorized(
    ...
    match name {
        ...
        "propose_steps" => {
            ...
            db.atomic(|| {
                ...
                db.event(
                    oid,
                    "workflow.proposed",
                    json!({"worker":wid,"revision":revision,"steps":new_ids,"warnings":warnings,"integrated_head":crate::decision::review::observed_head(db,oid)}),
                )?;
        ...
        "add_steps" => {
            ...
            template::validate(&plan.steps)?;
            crate::skills::validate_steps(&crate::skills::packet(db, oid)?, &plan.steps)?;
            db.atomic(||{crate::delegation::invalidate_acceptance(db,oid)?;for s in &added{db.conn.execute("INSERT INTO steps(id,task,name,spec,state) VALUES(?,?,?,?,'pending')",rusqlite::params![id(),oid,s.id,serde_json::to_string(s)?])?;}let rev:i64=db.conn.query_row("SELECT COALESCE(MAX(revision),0)+1 FROM revisions WHERE task=?",[oid],|r|r.get(0))?;let serialized=serde_json::to_string(&plan)?;db.conn.execute("INSERT INTO revisions VALUES(?,?,?,?)",rusqlite::params![oid,rev,serialized,now()])?;db.conn.execute("UPDATE tasks SET plan=?,status=CASE WHEN status='succeeded' THEN 'running' ELSE status END WHERE id=?",rusqlite::params![serialized,oid])?;db.event(oid,"workflow.revised",json!({"revision":rev,"warnings":warnings,"integrated_head":crate::decision::review::observed_head(db,oid)}))?;Ok(json!({"revision":rev,"warnings":warnings}))})
        }
        "put_artifact" => {
